...
import asyncio
import threading as _threading_top  # also imported below for health server
...
async def post_to_twitter(text: str) -> None:
    ...
    try:
        ...
        print("[TWITTER] tweepy not installed, skipping tweet.")
...
MAJOR_ARCANA_I18N: list[dict[str, str]] = [
    ...
    {"pl": "Wstrzemięźliwość", "en": "Temperance", "de": "Die Mäßigkeit"},
...
async def cmd_tarot(interaction: discord.Interaction):
    ...
    _tarot_header = f"\U0001f52e Karta dnia: {card} ({position})\n\n"
    _tarot_footer = "\n\n#tarot #ezoteryka #ezostylia #kartadnia\nezostylia.com"
    _tarot_max_ai = 280 - len(_tarot_header) - len(_tarot_footer) - 5
    _tarot_snippet = ai_text[:_tarot_max_ai] if len(ai_text) > _tarot_max_ai else ai_text
    tweet_text = f"\U0001f52e Karta dnia: {card} ({position})\n\n{_tarot_snippet}...\n\n#tarot #ezoteryka #ezostylia #kartadnia\nezostylia.com"
    asyncio.create_task(post_to_twitter(tweet_text))
...
async def cmd_runa(interaction: discord.Interaction):
    ...
    await interaction.followup.send(embed=embed)
    # Auto-post to Twitter/X
    _runa_header = f"\u2694\ufe0f Runa dnia: {symbol} {name}\n\n"
    _runa_footer = "\n\n#runa #ezoteryka #ezostylia #RunaDnia\nezostylia.com"
    _runa_max_ai = 280 - len(_runa_header) - len(_runa_footer) - 5
    _runa_snippet = ai_text[:_runa_max_ai] if len(ai_text) > _runa_max_ai else ai_text
    tweet_text = f"\u2694\ufe0f Runa dnia: {symbol} {name}\n\n{_runa_snippet}...\n\n#runa #ezoteryka #ezostylia #RunaDnia\nezostylia.com"
    asyncio.create_task(post_to_twitter(tweet_text))
...
# ---------------------------------------------------------------------------
@tree.command(name="twitter_wlacz", description="[ADMIN] Wlacz auto-postowanie na Twitter/X")
@app_commands.checks.has_permissions(administrator=True)
async def cmd_twitter_on(interaction: discord.Interaction):
    global _twitter_enabled
    if not (TWITTER_API_KEY and TWITTER_API_SECRET and TWITTER_ACCESS_TOKEN and TWITTER_ACCESS_SECRET):
        await interaction.response.send_message("\u274c Brak kluczy Twitter API w konfiguracji serwera.", ephemeral=True)
        return
    _twitter_enabled = True
    await interaction.response.send_message("\u2705 Auto-postowanie na Twitter/X **wlaczone**.", ephemeral=True)


@tree.command(name="twitter_wylacz", description="[ADMIN] Wylacz auto-postowanie na Twitter/X")
@app_commands.checks.has_permissions(administrator=True)
async def cmd_twitter_off(interaction: discord.Interaction):
    global _twitter_enabled
    _twitter_enabled = False
    await interaction.response.send_message("\u2705 Auto-postowanie na Twitter/X **wylaczone**.", ephemeral=True)
